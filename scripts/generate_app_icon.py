...
OUT = Path("assets/app_icon_512.png")
...
def main() -> None:
    ...
    draw.rounded_rectangle([0, 0, size, size], radius=80, fill="#6C63FF")
    draw.rounded_rectangle([20, 20, size - 20, size - 20], radius=68, fill="#5a52d5")

    # Diamond gem shape (centred)
    cx, cy = size // 2, size // 2 - 30
    gem_w, gem_h = 200, 180
    top    = (cx, cy - gem_h // 2)
    left   = (cx - gem_w // 2, cy + 20)
    right  = (cx + gem_w // 2, cy + 20)
    bottom = (cx, cy + gem_h // 2 + 10)
    tl     = (cx - gem_w // 4, cy - gem_h // 2 + 30)
    tr     = (cx + gem_w // 4, cy - gem_h // 2 + 30)

    gem_poly = [top, tr, right, bottom, left, tl]
    draw.polygon(gem_poly, fill="white")

    # Inner highlight triangle
    inner = [(cx, cy - gem_h // 2 + 10), tr, tl]
